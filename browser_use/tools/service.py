...
""".strip()
		...
		@self.registry.action(
			"""Scan visible viewport text and rank candidates by relevance to query. Zero LLM cost. Returns visible text snippets with tag, position, direction, and useful attributes. Use after find_text/search_page/find_elements to decide where to click, scroll, extract, or inspect next instead of repeating the same search.""",
			param_model=ScanVisibleTextAction,
		)
		async def scan_visible_text(params: ScanVisibleTextAction, browser_session: BrowserSession):
			js_code = r"""
(function() {
	try {
		...
		const tags = 'a,button,input,textarea,select,label,th,td,caption,h1,h2,h3,h4,h5,h6,p,li,span,div,strong,b,em';
		...
		function cssHint(el) {
			...
			if (href) return el.tagName.toLowerCase() + '[href="' + href.slice(0, 120).replace(/"/g, '\\"') + '"]';
		...
		for (const el of Array.from(document.querySelectorAll(tags))) {
			...
			if (['A', 'BUTTON', 'INPUT', 'SELECT', 'TEXTAREA'].includes(el.tagName)) score += 6;
			if (['TH', 'TD', 'CAPTION'].includes(el.tagName)) score += 4;
			if (/(table|grid|row|cell|link|button)/i.test(el.getAttribute('role') || '')) score += 3;
			if (/\d/.test(text)) score += 2;
			score += Math.max(0, 8 - Math.abs((rect.top + rect.height / 2) - window.innerHeight / 2) / 120);
			const cy = rect.top + rect.height / 2;
			let direction = 'current';
			if (cy < window.innerHeight * 0.25) direction = 'up';
			else if (cy > window.innerHeight * 0.75) direction = 'down';
			items.push({
		...
		items.sort((a, b) => b.score - a.score || a.y - b.y);
		return {query: QUERY, viewport, total: items.length, items: items.slice(0, MAX_RESULTS)};
	} catch (e) {
		return {error: 'scan_visible_text error: ' + e.message, items: [], total: 0};
	}
})()
""".replace('__QUERY__', json.dumps(params.query)).replace('__MAX_RESULTS__', str(int(params.max_results)))
			...
			if result.get('exceptionDetails'):
				error_text = result['exceptionDetails'].get('text', 'Unknown JS error')
				return ActionResult(error=f'scan_visible_text failed: {error_text}')

			data = result.get('result', {}).get('value')
			if data is None:
				return ActionResult(error='scan_visible_text returned no result')
			if isinstance(data, dict) and data.get('error'):
				return ActionResult(error=f'scan_visible_text: {data["error"]}')

			items = data.get('items', []) if isinstance(data, dict) else []
			lines = [
				f'Visible text scan for query "{params.query}" returned {len(items)} of {data.get("total", 0)} candidates.',
				'Use high-score candidates to choose the next click, scroll direction, selector inspection, or extraction target.',
			]
			for i, item in enumerate(items, 1):
				attrs = []
				if item.get('role'):
					attrs.append(f'role={item["role"]}')
				if item.get('href'):
					attrs.append(f'href={item["href"]}')
				if item.get('selector'):
					attrs.append(f'selector={item["selector"]}')
				attr_text = f' [{"; ".join(attrs)}]' if attrs else ''
				lines.append(
					f'{i}. score={item.get("score")} dir={item.get("direction")} y={item.get("y")} '
					f'tag={item.get("tag")} text="{item.get("text")}"{attr_text}'
				)

			formatted = '\n'.join(lines)
			memory = f'Scanned visible text: {len(items)} candidates for "{params.query}".'
			logger.info(f'?뵊 {memory}')
			return ActionResult(extracted_content=formatted, long_term_memory=memory)
